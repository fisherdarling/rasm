...
use crate::types::{Value, Data, index::{Align, Offset}};
...
impl fmt::Debug for MemInst {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "MemInst {{ data: {:?} Pages, min: {:?}, max: {:?} }}", self.data.len() / PAGE_SIZE, self.min, self.max)
    }
}
...
impl MemInst {
    ...
    pub fn mem_grow(&mut self, num_pages: i32) -> Result<i32, Error> {
        ...
        self.data.try_reserve_exact(num_pages * PAGE_SIZE).map_err(Error::MemoryGrow)?;
