use wabt::script::{ScriptParser, Command, CommandKind, Action, Value as WastValue};
            ...
            CommandKind::AssertReturn { action, expected } => {
                match action {
                    Action::Invoke { field, args, .. } => {
                        ...
                        let expected: Vec<WasmResult> = translate_wast_values(expected).into_iter().map(WasmResult::from).collect();
...
fn print_passed(line: u64, field: &str, verbose: bool) {
    if !verbose {
        println!("[{:04}] {} {:<30}", line, "[PASSED]".green(), &field.white());
    } else {
        println!("[{:04}] {} {:<30}", line, "[PASSED]".green(), &field.white());
    }
}
...
fn print_failed_hard(line: u64, field: &str, args: Vec<Value>, expected: Vec<WasmResult>, result: Result<WasmResult, Error>, verbose: bool) {
    if verbose {
        println!("[{:04}] {} {:<30} Args: {:?}, || Expected: {:?}, || Received: {:?}", line, "[FAILED]".red().underline().bold(), &field.yellow(), args, expected, result);                    
    } else {
        println!("[{:04}] {} {:<30}", line, "[FAILED]".red().underline().bold(), &field.yellow());
    }
}
